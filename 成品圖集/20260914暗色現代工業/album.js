...
const url=p=>new URL(p,base).href,roomLabel=(id,v=version)=>id==='collection'&&['v3','v4'].includes(v)?'開放收藏收納':data.rooms.find(r=>r.id===id)?.name||id;
...
function photo(e,type,full=false){const im=document.createElement('img');im.src=url(type==='ai'&&!full?e.thumb:e[type]);im.alt=e.version.toUpperCase()+' '+roomLabel(e.room,e.version)+'・方向 '+e.angle+'・'+(type==='ai'?'AI 效果':'原始 3D');im.loading='lazy';im.decoding='async';im.width=1536;im.height=1024;im.onerror=()=>{const p=document.createElement('p');p.className='error';p.textContent='圖片未能載入，請確認圖集與圖片資料夾一起保存。';im.replaceWith(p);};return im;}
...
function draw(){
 ...
 $('room').value=room;$('room').querySelector('option[value="collection"]').textContent=roomLabel('collection');
 ...
 for(const r of data.rooms){const entries=visible.filter(e=>e.room===r.id);if(!entries.length)continue;
  ...
  const title=document.createElement('h2');title.textContent=roomLabel(r.id);const sub=document.createElement('span');sub.textContent='三個方向 / A · B · C';head.append(title,sub);
  const grid=document.createElement('div');grid.className='photos';
  for(const e of entries){const b=button('',()=>open(e));b.className='photo';b.append(photo(e,mode));const cap=document.createElement('div');cap.className='caption';const label=document.createElement('span');label.textContent='方向 '+e.angle;const small=document.createElement('small');small.textContent='放大・對照 ↗';cap.append(label,small);b.append(cap);grid.append(b);}
  section.append(head,grid);$('gallery').append(section);
 }syncURL();
}
function detail(){if(!selected)return;$('detailTitle').textContent=roomLabel(selected.room,selected.version)+'・方向 '+selected.angle;$('detailVersion').textContent=selected.version.toUpperCase()+' / '+data.versions.find(v=>v.id===selected.version).name;
...
for(const r of [{id:'all',name:'全屋空間'},...data.rooms]){const o=document.createElement('option');o.value=r.id;o.textContent=r.name;$('room').append(o);}
 ...
 $('provenance').textContent=data.entries.length+' 個模型視角 × AI 效果對照；'+data.uniqueImages+' 張不同畫面。四版完全相同的取景共用圖檔。';draw();const initial=data.entries.find(e=>e.key===params.get('view'));if(initial&&visible.some(e=>e.key===initial.key))open(initial);
})();
