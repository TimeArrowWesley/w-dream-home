...
function draw(){
 ...
 for(const r of data.rooms){const entries=visible.filter(e=>e.room===r.id);if(!entries.length)continue;
  ...
  const title=document.createElement('h2');title.textContent=roomLabel(r.id);const sub=document.createElement('span');sub.textContent=r.id==='entry'?'門內入口 · ← 左 / ↖ 左前 / ↑ 正前 / ↗ 右前 / → 右':'五個方向 / A · B · C · D · E';head.append(title,sub);
