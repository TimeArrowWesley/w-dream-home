...
D.cabs.filter(c=>c.n!=='玄關 鞋櫃'&&!(/廚房|床頭|書房|更衣室|收藏室 A|收藏室 B|收藏室 C|電視牆|頂天包冰箱|投影機|傘架/.test(c.n))).forEach(c=>{const entry=/玄關/.test(c.n),side=entry?'S':'E';const m=cabinet(c.x,entry?755:c.y,c.w,entry?48:c.d,c.z0,layout.isV2&&entry?90-c.z0:c.z1-c.z0,c.n,side);m.userData.source=entry?'四版玄關完成面對齊修正':'既有 HTML 規劃';m.userData.desc=entry?`${c.w}寬 × 50外深 × ${m.parent.userData.cabinet.h}高 cm；48cm櫃身＋2cm客廳面封板，完成面與玻璃展示櫃齊平，玄關側仍在原y803線。`:c.s.replace(/<[^>]*>/g,' ');});
...
ctxIsland();
function ctxIsland(){ EQ.appliances(true,is,IS); }
...
basin(-75,423,130,60,'N');EQ.product('nx',89,436,0,'N',fittings,'主浴：排水中心距後牆30.5cm，110V；排水點與淋浴門依現場核對。');glazing(140,373,4,100,0,220);openDoor(140,300,73,'V',true,'主浴淋浴玻璃門',-.9);box(155,313,139,151,.4,1,M.grey);const tubShape=new T.Shape();tubShape.absellipse(0,0,35,75.5,0,Math.PI*2,false);const hole=new T.Path();hole.absellipse(0,0,29,67,0,Math.PI*2,true);tubShape.holes.push(hole);const tub=new T.Mesh(new T.ExtrudeGeometry(tubShape,{depth:47,bevelEnabled:true,bevelSize:1,bevelThickness:1,bevelSegments:3,curveSegments:32}),M.ceramic);tub.rotation.x=-Math.PI/2;tub.position.copy(pos(247,390,0));fittings.add(tub);info(tub,'浴缸','依平面浴缸區配置；72×153cm依清單外包络；型號與現況龍頭待確認。');ball(247,390,9,32,66,3,M.white);box(161,310,2,2,80,132,M.steel);box(161,310,16,2,210,2,M.steel);cyl(169,311,208,8,2,M.steel);
