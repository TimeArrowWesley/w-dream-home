...
const blockGrid=new Map(),CELL=100,cellKey=(x,z)=>Math.floor(x/CELL)+','+Math.floor(z/CELL);
...
function gather(){V.scene.updateMatrixWorld(true);blocks=[];doors=[];blockGrid.clear();for(const group of [V.architecture,V.fittings])group.traverse(m=>{if(!m.isMesh||m.userData.dynamicDoor||m.userData.allowance)return;for(let p=m;p;p=p.parent)if(!p.visible)return;const box=new T.Box3().setFromObject(m);if(box.max.y<12||box.min.y>180)return;if(m.userData.walkDoor){doors.push({mesh:m,box});return;}blocks.push(box);
 // Index an expanded footprint so a player checks only the current cell.
 for(let x=Math.floor((box.min.x-radius)/CELL);x<=Math.floor((box.max.x+radius)/CELL);x++)for(let z=Math.floor((box.min.z-radius)/CELL);z<=Math.floor((box.max.z+radius)/CELL);z++){const key=x+','+z;if(!blockGrid.has(key))blockGrid.set(key,[]);blockGrid.get(key).push(box);}
 });}
function free(x,z){if(!inside(x-radius,z)||!inside(x+radius,z)||!inside(x,z-radius)||!inside(x,z+radius))return false;if(window.HOME_INTERACTION&&HOME_INTERACTION.blocksPoint(x,z,radius))return false;return !(blockGrid.get(cellKey(x,z))||[]).some(b=>{const dx=x-Math.max(b.min.x,Math.min(x,b.max.x)),dz=z-Math.max(b.min.z,Math.min(z,b.max.z));return dx*dx+dz*dz<radius*radius;});}
