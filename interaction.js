...
for(const leaf of leaves){const meta=leaf.userData.swingFront,side=meta.face==='E'||meta.face==='W';leaf.geometry.computeBoundingBox();const bb=leaf.geometry.boundingBox,pivot=new T.Group();pivot.position.copy(leaf.position);if(side)pivot.position.z+=meta.hinge==='max'?bb.max.z:bb.min.z;else pivot.position.x+=meta.hinge==='max'?bb.max.x:bb.min.x;const front=meta.hingeFrontOffset||0,edge=meta.hingeEdgeOffset||0;if(side){pivot.position.x+=(meta.face==='E'?1:-1)*front;pivot.position.z+=(meta.hinge==='max'?1:-1)*edge;}else{pivot.position.z+=(meta.face==='S'?1:-1)*front;pivot.position.x+=(meta.hinge==='max'?1:-1)*edge;}leaf.parent.add(pivot);leaf.position.sub(pivot.position);pivot.add(leaf);let angle={S:-1,N:1,E:1,W:-1}[meta.face]*1.48;if(meta.hinge==='max')angle*=-1;register(pivot,{name:meta.name+'・櫃門',openAngle:angle},leaf);
...
// Cabinet bypass panels are independent controls with an explicit travel limit.
const slidingFronts=[];V.fittings.traverse(o=>{if(o.isMesh&&o.userData.slideFront)slidingFronts.push(o);});
for(const leaf of slidingFronts){const meta=leaf.userData.slideFront,pivot=new T.Group();leaf.parent.add(pivot);pivot.add(leaf);const e=register(pivot,{name:meta.name,openAngle:1},leaf);e.axis=meta.axis;e.distance=meta.distance;e.slides=[{m:leaf,start:leaf.position[e.axis],ratio:1}];pivot.userData.slidingCabinet=true;}
