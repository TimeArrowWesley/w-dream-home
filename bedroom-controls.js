...
(()=>{
 ...
 function light(){const p=presets[state.scene],tint=['daily','makeup'].includes(state.scene)?'#f5f7ff':state.scene==='reading'?'#ffe2bd':'#ffbd72';for(const l of V.finishContext.roomLights){const x=l.position.x+482.5,y=l.position.z+480;if(x>=-75&&x<405&&y<282){l.intensity=p.ambient;l.color.set(tint).convertSRGBToLinear();}}
  B.task.color.set(tint).convertSRGBToLinear();B.closetTask.color.set('#f5f7ff').convertSRGBToLinear();
  for(const [type,meshes] of Object.entries(B.diffusers))for(const m of meshes){const hex=['mirror','closet'].includes(type)?'#f5f7ff':tint;m.material.color.set(hex).convertSRGBToLinear();m.material.emissive.set(hex).convertSRGBToLinear();}
