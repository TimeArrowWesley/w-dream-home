...
[本次前後圖與核對紀錄](調整紀錄/20260911全屋核對與主臥細節/index.html)。`node tools/verify-private-upgrade.cjs` 檢查四版尺寸、家具收放、窗簾、玻璃門及儲藏拉門通行；`verify-master-vanity.cjs` 已轉接至此共用驗證。可選用 `--before <本機快照.json>` 做前後幾何範圍比對，快照不發佈。圖片為離線CPU檢查；尚未實機驗證GPU。海豚連推車收納待最大外徑確認，層架未先行拆改。
