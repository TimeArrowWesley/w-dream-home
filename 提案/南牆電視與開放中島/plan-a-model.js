'use strict';
// Approved V4 floor plan. Shared private rooms, finishes and storage stay in their builders.
(()=>{
const shared=window.HOME_OPEN_ISLAND_BUILD;
window.HOME_OPEN_ISLAND_BUILD={
 storage(ctx){
  const result=shared.storage(ctx);
  ctx.fittings.traverse(o=>{o.name=o.name.replace(/^V3 /,'V4 ');for(const k of ['name','desc','source'])if(typeof o.userData[k]==='string')o.userData[k]=o.userData[k].replaceAll('V3','V4');if(o.userData.swingFront)o.userData.swingFront.name=o.userData.swingFront.name.replaceAll('V3','V4');});
  return result;
 },
 island({T,M,pos,box,info:baseInfo,EQ}){
  ...
  const g=EQ.group('V4 開放長中島'),spec=window.HOME_OPEN_ISLAND_SPEC;
  g.userData.footprint={...spec.island};g.userData.island={version:'v4',opening:'W',sink:spec.sink,knee:spec.kneeRecess};
  ...
  info(top,'V4 280×110×H95 開放長中島','酒櫃、掃地機、IH、濕區由北至南分艙，開口朝西／冰箱。石材真開孔，備餐單槽55×45、深20；東側留膝約33×205，南端完整木皮。');
  ...
  panel(425,480,110,1.8,0,92,'V4 中島北端固定板',true);
  for(const y of [550,602,670])panel(426.8,y,71.4,1.8,0,92,'V4 分艙固定側板');
  panel(425,758.2,110,1.8,8,84,'V4 玄關端完整木皮',true);
  panel(429,754.2,102,1.8,0,8,'V4 玄關端內縮踢腳');
  panel(533.2,481.8,1.8,71.8,8,84,'V4 東側酒櫃背面木皮',true);
  panel(529.2,485.8,1.8,63.8,0,8,'V4 酒櫃背面內縮踢腳');
  panel(498.2,551.8,35,1.8,8,84,'V4 酒櫃與留膝側面轉折封板',true);
  panel(502.2,547.8,27,1.8,0,8,'V4 轉折內縮踢腳');
  panel(498.2,553.6,1.8,204.6,8,84,'V4 東側留膝完成面',true);
  panel(494.2,555.8,1.8,198.4,0,8,'V4 留膝背面內縮踢腳');
  panel(429,605.8,1.8,62.4,0,8,'V4 IH 艙內縮踢腳');
  panel(429,673.8,1.8,82.4,0,6.2,'V4 濕區落地內縮踢腳');
  panel(427,671.8,71.2,86.4,6.2,1.8,'V4 濕區設備承重底板');
  for(const y of [553.6,621.8,689.8,756.2])box(500,y,33.2,2,89,3,M.steel,g,'V4 留膝檯面橫向支撐');
  box(531.2,555.6,2,200.6,89,3,M.steel,g,'V4 留膝檯面外緣支撐');
  ...
  panel(427,551.8,71.2,50.2,40,1.8,'V4 掃地機上方檢修層板');
  panel(475.5,551.8,1.8,50.2,0,40,'V4 掃地機後側隔板');
  panel(425,552.1,1.8,49.6,42,49.7,'V4 掃地機上方收納門',true).userData.swingFront={name:'V4 掃地機上方收納',face:'W',hinge:'min'};
  EQ.product('ih',457.5,636.9,90.3,'N',g,'27×49.5cm真開孔；爐面95.4cm，與石材分高避免閃爍。下方獨立通風層。');
  panel(427,603.8,71.2,66.2,76,1.8,'V4 IH 獨立散熱隔板');
  panel(425,604.1,1.8,65.6,8,66,'V4 IH下方乾式收納門',true).userData.swingFront={name:'V4 IH下方乾式收納',face:'W',hinge:'min'};
  panel(425,603.8,1.8,66.2,86.5,5.5,'V4 IH通風帶上收口',true);
  for(const y of [603.8,669])panel(425,y,1.8,1,74,12.5,'V4 IH通風帶側框');
  for(const z of [77,80,83])panel(426,604.8,1.8,64.2,z,.7,'V4 IH西側內縮散熱葉片');
  ...
  function flat(s,z,h,mat,name,role){const m=new T.Mesh(new T.ExtrudeGeometry(s,{depth:h,bevelEnabled:false,curveSegments:12}),mat);m.rotation.x=-Math.PI/2;m.position.y=z;m.castShadow=m.receiveShadow=true;m.userData.islandSurface=role;info(m,name,'V4 備餐槽外框55×45、內槽50×40、深20cm；產品型號及開孔模板待選定。');g.add(m);return m;}
  const rim=rounded(455,725,45,55,4.5);rim.holes.push(rounded(455,725,40,50,3.5));flat(rim,95.02,.14,metal,'V4 備餐單槽薄邊','sink-rim');
  ...
  const bowl=new T.Mesh(geo,metal);bowl.userData.islandSurface='sink-bowl';info(bowl,'V4 備餐槽中空內膽','薄壁漸縮，20cm真槽深；內部與排水口保持中空。');g.add(bowl);
  const floor=rounded(455,725,34,44,3.5),drainHole=new T.Path();drainHole.absellipse(455-482.5,480-725,4.5,4.5,0,Math.PI*2,true);floor.holes.push(drainHole);flat(floor,74.5,.5,metal,'V4 槽底真排水口','sink-floor');
  const drain=new T.Shape();drain.absellipse(455-482.5,480-725,5.1,5.1,0,Math.PI*2,false);const inner=new T.Path();inner.absellipse(455-482.5,480-725,4.1,4.1,0,Math.PI*2,true);drain.holes.push(inner);flat(drain,75.03,.15,metal,'V4 排水薄邊','sink-drain');
  ...
  tube([[455,725,74.5],[455,725,51],[455,732,31],[455,741,32],[449,741,43],[432,741,43]],2,metal,'V4 存水彎與排水');
  tube([[448,687,95],[448,687,116],[448,691,126],[448,705,126],[448,710,116]],1.3,M.steel,'V4 備餐冷熱水龍頭');
  tube([[470,687,95],[470,687,121],[470,689,126],[470,704,126],[470,709,122]],.85,M.black,'V4 CLAR獨立飲水龍頭');
  box(468,684,4,3,97,6,M.screen,g,'V4 CLAR觸控溫度面板');
  for(const [x,name] of [[448,'冷熱水'],[470,'CLAR飲水']]){box(x-2,685,4,4,94.99,.4,M.steel,g,'V4 '+name+'固定座');tube([[x,687,94.9],[x,687,62],[x,679,48]],.5,M.black,'V4 '+name+'供水管');}
  for(const [y,d] of [[672.1,42.75],[715.15,42.75]])panel(425,y,1.8,d,8,83.7,'V4 濕區西向檢修門',true).userData.swingFront={name:'V4 飲水濕區檢修',face:'W',hinge:'min'};
  for(const [name,x,y,w,d,z,h,note] of [
   ['V4 酒櫃開放艙',427,481.8,106.2,68.2,0,92,'獨立式酒櫃；安裝條件待原廠確認'],
   ['V4 掃地機維修艙',427,551.8,48.5,50.2,0,40,'西側抽出檢修'],
   ['V4 IH乾式散熱艙',427,603.8,71.2,66.2,77.8,14.2,'與下櫃及濕區分隔'],
   ['V4 飲水濕區',427,671.8,71.2,86.4,8,84,'水槽、飲水機、存水彎和雙扇檢修門']])EQ.bay(name,x,y,w,d,z,h,note);
  EQ.allowance('V4 酒櫃西向操作預留',360,482,65,68,0,90,'避免與冰箱同時對開');
  EQ.allowance('V4 掃地機西向進出預留',365,551.8,60,50.2,0,35,'回充須依實機測試');
 ...
 living({T,M,pos,box,cyl,info,EQ,finishHeight,wallParts}){
  ...
  const wall=EQ.group('V4 南牆固定電視完成面');
  const wallHeight=finishHeight(917.5,951),backing=box(760,951,315,2,0,wallHeight,M.concrete,wall,'V4 南牆灰礦物塗料背牆');
  ...
  const stools=[];for(const y of [610,710]){
   ...
   cyl(570,y,61,20,4,M.cloth,g);for(const x of [558,582])for(const yy of [y-12,y+12])box(x-1,yy-1,2,2,0,61,M.steel,g,'V4 活動椅落地腳');
