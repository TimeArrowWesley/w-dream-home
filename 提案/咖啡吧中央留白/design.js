...
function cabinet(x,y,w,d,z,h,name,face='S',finish=M.blackglass){if(z+h>=218)h=Math.max(h,finishHeight(x+w/2,y+d/2)-z);const g=new T.Group();g.userData.cabinet={name,w,d,h};fittings.add(g);const side=face==='E'||face==='W',th=1.8;const carc=box(side?(face==='E'?x:x+w-th):x,side?y:(face==='S'?y:y+d-th),side?th:w,side?d:th,z,h,M.black,g,name,`${w} × ${d} × ${h} cm；霧黑櫃身＋黑玻門片；櫃內層板為示意。`);if(side){box(x,y,w,th,z,h,M.black,g);box(x,y+d-th,w,th,z,h,M.black,g);}else{box(x,y,th,d,z,h,M.black,g);const rightSide=box(x+w-th,y,th,d,z,h,M.black,g);if(name==='玄關 外套櫃')rightSide.name='玄關外套櫃固定右側板';}box(x,y,w,d,z,th,M.black,g);box(x,y,w,d,z+h-th,th,M.black,g);for(let zz=z+38;zz<z+h-15;zz+=38)box(x+th,y+th,w-2*th,d-2*th,zz,th,M.black,g);const n=Math.max(1,Math.round((side?d:w)/60));
...
collectionDoor.parent.userData.interactiveDoor.openAngle=-1.15;
...
D.cabs.filter(c=>c.n!=='玄關 鞋櫃'&&!(/廚房|床頭|書房|更衣室|收藏室 A|收藏室 C|電視牆|頂天包冰箱|投影機|傘架/.test(c.n))).forEach(c=>{const entry=/玄關/.test(c.n),side=entry?'S':'E';const m=cabinet(c.x,c.y+(comfort&&entry?10:0),c.w,c.d-(comfort&&entry?10:0),c.z0,layout.isV2&&entry?90-c.z0:c.z1-c.z0,c.n,side);m.userData.source=layout.isV2&&entry?'V2 玄關矮櫃提案':'既有 HTML 規劃';m.userData.desc=layout.isV2&&entry?`${c.w}×${c.d-(comfort?10:0)}×90cm 矮櫃；${comfort?"舒適提案減深10cm，保留櫃背位置":"保留原櫃寬深"}，內部收納重新配置，原高櫃旋轉鞋架與吊衣機能不沿用。`:c.s.replace(/<[^>]*>/g,' ');});
...
box(675,763,35,2,0,entryLowTop,M.black,entryDaily);
for(const x of [675,708])box(x,763,2,40,0,entryLowTop,M.black,entryDaily);
box(675,763,35,40,entryLowTop-2,2,M.blackglass,entryDaily);
for(const z of [5,30,55]){box(677,765,31,36,z,1.8,M.steel,entryDaily,'玄關少量鞋層板');for(const x of [681,694])ball(x,784,z+5,4,12,4,M.darkcloth,entryDaily);}
...
panel(550,comfort?761:751,60,2,8,layout.isV2?82:267,M.black,'玄關面客廳封板');panel(610,comfort?761:751,100,2,8,entryLowTop-8,M.blackglass,'玄關低櫃背面');line(610,comfort?760:750,entryLowTop-2,100);
...
return g;}
// Trial: replace the left 180cm of the collection-room frontage with a recessed coffee bar.
// The back remains at y793; the new counter projects only 15cm beyond the old display.
const coffeeStart=fittings.children.length;
box(215,791,180,2,0,275,M.black,fittings,'咖啡吧固定背板');
cabinet(215,738,180,55,8,79,'咖啡吧地櫃','N',M.black);
box(220,742,170,47,0,8,M.black);
box(213,736,184,59,87,3,M.grey,fittings,'咖啡吧石材檯面','180cm內嵌咖啡吧，檯面高90cm、深約59cm；北向使用，中央走道保持淨空。');
for(const x of [215,393])box(x,738,2,55,0,275,M.black,fittings,'咖啡吧木飾側板');
cabinet(217,758,176,35,185,90,'咖啡吧吊櫃','N',M.black);
box(219,789,172,2,90,95,M.grey,fittings,'咖啡吧石材背板');
line(220,758,183,170,.8);
pointLight(300,770,170,.18,180).name='咖啡吧工作照明';
// Countertop appliances are illustrative, with a refillable water tank and no new sink.
box(235,751,36,29,90,39,M.steel,fittings,'咖啡吧義式咖啡機','檯面咖啡機示意，實品尺寸待選。');
box(237,750,32,1,98,27,M.blackglass);
box(238,741,30,12,92,2,M.steel);
for(const x of [242,258]){cyl(x,747,94,3,6,M.ceramic);box(x,749,1.5,4,107,2,M.steel);}
box(238,750,8,1,118,4,M.screen);cyl(255,761,129,5,5,M.black);
box(347,758,21,23,90,37,M.black,fittings,'咖啡吧桌上飲水機','補水式桌上飲水機示意；不新增中島水槽，設備與供電位置待選定。');
box(349,757,17,1,107,17,M.blackglass);box(350,747,15,11,91,2,M.steel);box(355,756,3,4,106,2,M.steel);
cyl(357,752,93,3,8,M.ceramic);
box(291,751,32,22,90,1,M.steel,fittings,'咖啡吧備杯托盤');
for(const x of [298,312]){cyl(x,763,91,4,7,M.ceramic);cyl(x,754,91,4,7,M.ceramic);}
const coffeeAssembly=new T.Group();coffeeAssembly.name='收藏室外側內嵌咖啡吧';
const coffeeParts=fittings.children.slice(coffeeStart);fittings.add(coffeeAssembly);coffeeParts.forEach(p=>coffeeAssembly.add(p));
coffeeAssembly.userData.coffeeBar={width:180,depth:55,counterHeight:90,backY:793,frontY:738,centralClear:true};
display(395,753,65,40,245,'收藏室保留展示面','N');
...
info(wall(555,80,60,40,275),'更衣室管道間','按客變輪廓補入，60×40cm原圖配準輪廓，截面待複量。','客變P-02');wardrobe(580,0,110,55,'S');wardrobe(690,0,55,260,'W');wardrobe(580,205,110,55,'N');panel(582,207,58,1,20,180,M.mirror,'更衣鏡');{
 const low=cabinet(415,80,70,40,0,90,'更衣室窗前矮櫃','S');low.userData.desc='依業主黃框：70×40×90cm暫定，窗前維持矮櫃。';
 const tall=cabinet(485,80,70,40,0,275,'更衣室頂天櫃','S');tall.userData.desc='依業主綠框：70×40cm，櫃頂銜接目前天花完成面。';
}
...
{id:'living',n:'客廳・中央留白',en:'LIVING',p:[570,700,157],t:[961,651,113],note:'L 型沙發依紅框後移約70cm，貴妃段靠窗；移除中央中島及雙面玻璃櫃，保留開放活動與行走空間。',label:[925,700]},
{id:'island',n:'咖啡吧・留白',en:'COFFEE LOUNGE',p:[490,580,165],t:[305,762,135],note:'收藏室外側整合180cm咖啡吧；石材檯面、木飾櫃面與暖光，中央不放大型家具。',label:[305,746]},
...
{id:'closet',n:'更衣室',en:'WARDROBE',p:[632,180,155],t:[655,75,125],note:'北側窗、窗前90cm矮櫃與頂天櫃；V1/V2相同。窗寬70cm、窗台95cm及櫃深40cm依模型暫定。',label:[625,132]},
