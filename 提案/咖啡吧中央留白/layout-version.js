...
window.HOME_LAYOUT={comfort:true,version,isV2:version==='v2',entryCabinetHeight:version==='v2'?90:null,entryDoorY:955};
window.addEventListener('DOMContentLoaded',()=>{
 ...
 bar.innerHTML='<strong>獨立試作：咖啡吧＋中央留白</strong><span>玄關</span><button data-layout="v1">高矮櫃</button><button data-layout="v2">矮櫃</button><span id="layoutDescription"></span>';
 ...
 document.getElementById('layoutDescription').textContent=version==='v2'?'玄關全矮櫃90cm':'玄關外套高櫃＋120cm低櫃';
 const original=document.createElement('a');original.href='index.html?layout='+version;original.textContent='開啟原版（中島＋玻璃櫃）↗';original.target='_blank';bar.appendChild(original);const preview=document.createElement('a');preview.href='整理紀錄/中央留白試作/預覽.html';preview.textContent='試作靜態預覽';preview.target='_blank';bar.appendChild(preview);
 ...
 HOME_TOUR.setMode('model');HOME_VIEWER.selectRoom('island');for(const [id,value] of [['cut',false],['ceiling',true],['labels',false]]){const control=document.getElementById(id);control.checked=value;control.dispatchEvent(new Event('change'));}document.querySelector('.workspace').classList.remove('planhidden');document.getElementById('planToggle').textContent='收起平面圖';
});
})();
