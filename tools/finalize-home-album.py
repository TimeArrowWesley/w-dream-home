...
assert len(entries)==144 and len(keys)==82
...
provenance={'date':'2026-09-15','sourceRevision':manifest['modelRevision'],'sourceMethod':manifest['method'],'generationTool':'Built-in image_gen','viewSlots':144,'uniqueImages':82,'records':records}
...
report=read('成品核對.json');report.update({'allImagesDecoded':True,'all144SourceHashesMatched':True,'correctionViews':len(fixes),'visualReview':'Each unique AI image reviewed; original model is authoritative for dimensions','browserVisualQA':False})
...
out=project/'成品圖集/W夢想之家_四版全屋AI圖集_精簡包.zip'
with zipfile.ZipFile(out,'w',zipfile.ZIP_DEFLATED,compresslevel=6) as z:
 for p in files:
  name=p.relative_to(root).as_posix()
  if name=='index.html':
   html=p.read_text(encoding='utf8').replace('../../index.html','index.html').replace('<a href="../../方案比較.html">四版格局比較 ↗</a>','<span>四版全屋 · 離線成品圖集</span>')
...
with zipfile.ZipFile(out) as z:
 ...
 print(json.dumps({'zip':str(out),'files':len(files),'MB':round(out.stat().st_size/1048576,2),'imagesDecoded':246,'sourceHashesVerified':144},ensure_ascii=False))
