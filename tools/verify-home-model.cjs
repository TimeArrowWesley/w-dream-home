...
for(let n=1;n<=4;n++){
 ...
 if(process.env.HOME_VISUAL_ONLY){c.HOME_COMFORT?.setScene('daily');require('./render-home-review.cjs')({T,V,version:'v'+n,out:path.join(root,'調整紀錄/20260910全屋外觀檢視')});continue;}
 ...
 if(n===1){const out=path.join(root,'調整紀錄/20260910玄關與木皮');fs.mkdirSync(out,{recursive:true});const rgb=new Uint8Array(512*1024*3);for(let i=0;i<512*1024;i++){const tint=new T.Color(pixelData[i*4]/255,pixelData[i*4]/255,pixelData[i*4]/255).convertSRGBToLinear().multiply(veneer.color).convertLinearToSRGB();rgb[i*3]=Math.round(tint.r*255);rgb[i*3+1]=Math.round(tint.g*255);rgb[i*3+2]=Math.round(tint.b*255);}require('./render-home-review.cjs').png(path.join(out,'深灰棕直紋木皮.png'),512,1024,rgb);}
 ...
 report.variants['v'+n]={meshes:allMeshes.length,vertices,rooms,refinements:{...F,checks:{airPathRays:9,airVolumesOutsideWalls:true,grilleFreeArea:true,mirrorSweepStepCm:1,mirrorButton:true,displayEnvelopePreserved:true}},door:{closed,opened,travelCm:122,clearOpeningCm:120,minParkingGapCm:minGap,sweepStepCm:1,modelModeToggle:true,walkingBlocksClosed:true,walkingClearOpen:true,obstructionReopens:true},entryFaces:faces.map(o=>({name:o.userData.name,...bounds(o)})),entryFinish:{facadeRays,panels:beforeTexture.length,jointCm:.2,lateTextureCallbacksStable:true},sharedZones,wallJunctions,islandBackOcclusionRays:skinRays,supports,hvac,curtains:c.HOME_CURTAINS.getState(),rgbModes:5,finish:c.HOME_REALISM.getState()};
 report.variants['v'+n].finishRevision={entry:floorState.entry,floorRays,cabinetVeneer:veneerStats};
 console.log(`V${n}: ${allMeshes.length} meshes, ${rooms.length} room views, door sweep/controls, entry, island, supports, HVAC, RGB and curtains passed.`);
}
if(!process.env.HOME_VISUAL_ONLY){
 for(const n of [2,3,4])assert.equal(JSON.stringify(report.variants.v1.finishRevision),JSON.stringify(report.variants['v'+n].finishRevision),'shared foyer and dark cabinet finish V'+n);
 for(const [a,b] of [['v1','v2'],['v3','v4'],['v1','v3']])assert.deepStrictEqual(report.variants[a].sharedZones,report.variants[b].sharedZones,`${a}/${b} nine common rooms geometry and finish`);
 report.sharedRoomComparison='Nine common room mesh / geometry / material fingerprints match all four versions. Entry and the living/island layout are reviewed separately.';
