...
(async()=>{
 ...
 assert.equal(c.HOME_LAYOUT.version,'a');assert.equal(V.rooms.length,13);assert.equal(E.items.length,24);assert(c.HOME_FIXED_LIVING);assert(!c.HOME_ROTATING_TV);assert(!c.HOME_ROTATING_TV_CONTROLS);
 const backing=V.wallParts.find(p=>p.m.userData.name==='A 南牆灰礦物塗料背牆');assert(backing,'TV finish follows wall cutaway');
 ...
 const clar=E.items.find(o=>o.userData.equipment.key==='clar'),cb=bounds(clar),base=bounds(named('A 濕區設備承重底板'));near(cb.z,base.z+base.h,'CLAR supported');
 ...
 const faces=[];island.traverse(o=>{if(o.userData.name==='A 濕區西向檢修門')faces.push(bounds(o));});faces.sort((a,b)=>a.y-b.y);assert.equal(faces.length,2);near(faces[0].y-671.8,.3,'wet north reveal');near(faces[1].y-faces[0].y-faces[0].d,.3,'wet seam');near(758.2-faces[1].y-faces[1].d,.3,'wet south reveal');
 ...
 near(bounds(named('A 玄關端完整木皮')).y+1.8,760,'finished island end');const turn=bounds(named('A 酒櫃與留膝側面轉折封板'));near(turn.w,35,'wine-to-knee step completely capped');near(turn.y+turn.d,bounds(named('A 東側留膝完成面')).y,'step to knee board butt joint');
 near(760-bounds(named('A 玄關端內縮踢腳')).y-bounds(named('A 玄關端內縮踢腳')).d,4,'toe kick recess');
 ...
 const glass=[];V.fittings.traverse(o=>{if(o.name==='A 收藏玻璃展示櫃')o.traverse(m=>{if(m.userData.name==='A 玻璃展示門')glass.push([bounds(m),o.userData.glassOpening]);});});assert.equal(glass.length,3);for(const [q,o] of glass){near(q.z-o.z,.3,'glass bottom');near(o.z+o.h-q.z-q.h,.3,'glass top');}
