...
function fixture(v,params='') {
  const proposal=['v2','v3','v4'].includes(v),prefix=v==='v0'?'提案/原始格局/':v==='v4'?'提案/南牆電視與開放中島/':v==='v3'?'提案/開放大中島/':proposal?'提案/旋轉電視與直線中島/':'',modelPrefix=proposal?'提案/旋轉電視與直線中島/':'';
  ...
  const layout={isV2:v==='v1'||v==='v0',originalPlan:v==='v0'};
  const rooms=vm.runInNewContext(read(modelPrefix+'design.js').match(/const ROOMS=(\[[\s\S]*?\n\]);/)[1],{layout});
  if(v==='v0')for(const [id,n] of [['collection','貓房'],['study','書房・電子琴'],['living','客廳・弧形沙發']])rooms.find(r=>r.id===id).n=n;
  ...
  const equipment=geometry.variants[['v3','v4'].includes(v)?'v2':v==='v0'?'v1':v].equipment.map(e=>{const g=new T.Group();g.name=e.name;g.userData.equipment=e.expected;g.userData.desc=e.name;return g;});
...
for(const v of ['v0','v1','v2','v3','v4']){
  ...
  assert.equal($('layoutSwitch').children.length,5);assert.equal(d.querySelectorAll('#uiRoomList [data-id]').length,13);
  ...
  if(v==='v2'||v==='v3'){assert($('uiInspector').contains($('rotatingTVPanel')));d.querySelector('[data-tv-facing="island"]').click();assert(calls.some(q=>q[0]==='tv'&&q[1]==='island'));}
...
const deep=fixture('v2','&uiRoom=bed&uiMode=model');assert.equal(deep.c.HOME_UI.getState().room,'bed');checks.push('Switching versions retains selected room; normal startup returns to whole-home overview');
{
 ...
 for(const v of ['v0','v1','v2','v3','v4']){document.getElementById('tab-'+v).click();assert(document.getElementById('versionTitle').textContent.startsWith(v.toUpperCase()));}
 checks.push('Five design comparison tabs render correct names, metrics, drawings and 3D links');
}
...
checks.push('Full mesh geometry verified separately in V0 layout tests; five-design UI and shared furniture handlers preserved');
