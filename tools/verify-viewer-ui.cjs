...
function fixture(v,params='') {
  const registry=JSON.parse(read('version-registry.json')).versions,cfg=registry.find(x=>x.id===v),legacy=cfg.legacy,proposal=!['v0','v1'].includes(v),prefix=cfg.path.replace(/index.html$/,''),modelPrefix=proposal?'提案/旋轉電視與直線中島/':'';
  ...
  run('version-registry.js');run(prefix+'layout-version.js');
  ...
  run('equipment-controls.js');if(['v2','v3'].includes(v)){c.HOME_FIXED_LIVING={stools:[]};run('提案/南牆電視與開放中島/plan-a-controls.js');}run('furniture-core.js');run('furniture-data.js');run('furniture-app.js');
...
for(const v of ['v0','v1','v2','v3','v4','v5']){
  ...
  if(v==='v4'||v==='v5'){assert($('uiInspector').contains($('rotatingTVPanel')));d.querySelector('[data-tv-facing="island"]').click();assert(calls.some(q=>q[0]==='tv'&&q[1]==='island'));}
