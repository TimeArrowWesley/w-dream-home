...
data={'date':manifest.get('capturedAt','2026-09-16')[:10],'sourceRevision':manifest['modelRevision'],'versionSources':manifest.get('versionSources',{}),'method':manifest['method'],'generation':'Built-in image_gen; each unique source view individually edited','complete':not missing,'uniqueImages':len(jobs)-len(missing),'versions':[{'id':f'v{i}','name':n} for i,n in enumerate(titles)],'rooms':[{'id':r,'name':n} for r,n in names.items()],'entries':entries}
(root/'album-data.js').write_text('window.HOME_ALBUM='+json.dumps(data,ensure_ascii=False,separators=(',',':'))+';\n',encoding='utf8',newline='\n')
report={'complete':not missing,'viewSlots':len(entries),'uniqueImages':data['uniqueImages'],'remaining':missing,'versions':{f'v{i}':sum(e['version']==f'v{i}' for e in entries) for i in range(5)},'displayBytes':sum((root/folder/f'{key}.webp').stat().st_size for folder in ['images','models','thumbs'] for key in jobs if key not in missing)}
...
if '--install' in sys.argv:
 ...
 (project/'assets/ai-interiors/catalog.js').write_text('/* 2026-09-16: integrated sofa and bedside model refresh; five fixed-station entry directions; three directions per other room. Shared images require identical source pixels. */\nwindow.HOME_AI_PHOTOS='+json.dumps(catalog,ensure_ascii=False,indent=2)+';\n',encoding='utf8',newline='\n')
