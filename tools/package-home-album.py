...
data={'date':'2026-09-15','sourceRevision':manifest['modelRevision'],'method':manifest['method'],'generation':'Built-in image_gen; each unique source view individually edited','complete':not missing,'uniqueImages':len(jobs)-len(missing),'versions':[{'id':f'v{i+1}','name':n} for i,n in enumerate(titles)],'rooms':[{'id':r,'name':n} for r,n in names.items()],'entries':entries}
(root/'album-data.js').write_text('window.HOME_ALBUM='+json.dumps(data,ensure_ascii=False,separators=(',',':'))+';\n',encoding='utf8',newline='\n')
report={'complete':not missing,'viewSlots':len(entries),'uniqueImages':data['uniqueImages'],'remaining':missing,'versions':{f'v{i}':sum(e['version']==f'v{i}' for e in entries) for i in range(1,5)},'displayBytes':sum(p.stat().st_size for folder in ['images','models','thumbs'] for p in (root/folder).glob('*.webp'))}
...
if '--install' in sys.argv:
 ...
 for v in ['v1','v2','v3','v4']:
  catalog[v]=[{'id':e['key'],'room':e['room'],'label':('開放收藏收納' if e['room']=='collection' and v in ['v3','v4'] else names[e['room']])+'・方向 '+e['angle'],'src':prefix+e['ai'],'modelSrc':prefix+e['model'],'thumb':prefix+e['thumb']} for e in entries if e['version']==v]
