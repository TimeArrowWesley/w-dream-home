...
for(const page of ['index.html','提案/原始格局/index.html','提案/旋轉電視與直線中島/index.html','提案/開放大中島/index.html','提案/南牆電視與開放中島/index.html']){
 const html=read(page);assert(html.includes('equipment-models.js?v=20260921-gbath-r04'));assert(html.includes('furniture-data.js?v=20260921-gbath-r04'));assert(html.includes('catalog.js?v=20260921-gbath-r04'));
}
...
const before='調整紀錄/20260921客浴原位收納/album-before.json';if(fs.existsSync(before)){const old=JSON.parse(read(before));assert.deepEqual(m.entries.filter(e=>e.room!=='bath2'),old.entries.filter(e=>e.room!=='bath2'));}
console.log(JSON.stringify({catalogRows:d.items.length,originalSourceRows:imported.length,newPendingItems:2,historicalShowerArchived:true,synchronizedEntries:5,guestBathroomViewSlots:15,uniqueNewImages:5,otherAlbumEntriesRetained:175}));
