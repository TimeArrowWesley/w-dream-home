...
async function main(){
 ...
 for(const q of before.entries.filter(e=>e.version==='v4')){
  ...
  const id=q.room+'-'+(q.room==='entry'?'door5':'')+q.angle+'-v4gi01';
  ...
  records.push({...q,...rec,model:'model/'+rec.file,modelHash:hash,changed:hash!==q.modelHash||q.room==='study',lightingAffected:q.room==='study',previousModelHash:q.modelHash,previousAI:q.aiKey,sourceRevision:'20260923-v4gi01',status:'source-captured'});
