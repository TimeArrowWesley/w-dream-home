...
for e in m['entries']:
 ...
 updated.append({**c,'ai':f'originals/{j["key"]}.png','aiKey':j['key'],'status':'ai-reviewed','sharedWith':j['mappedViews'],'lastReviewed':'2026-09-24'})
m.update({'entries':updated,'modelRevision':'20260924-tw01+au01+bi01','imageRevision':'20260924-tw01+au01+bi01','imagesUpdatedAt':'2026-09-24'})
m['versionSources']['TW01']={'date':'2026-09-24','versions':['v1'],'sourceFileSha256':{f:sha(P/f) for f in ['tv-wall-unification.js','grey-stone-design.js']},'basis':'Owner adopts V4 full-height gray-stone TV feature without overhead cabinet in V1. Lower AV console and electrical service retained; AU01 positioning preserved. Model dimensions, stone fixing and service details pending.','recapturedSlots':60,'newNativeAI':len(jobs),'updatedSlots':sum(c['changed'] for c in cs)}
...
write(A/'視覺核對.json',{**oldq,'revision':m['imageRevision'],'reviewedFinalImages':sorted(current),'reviewedCorrections':sorted(allfix),'latestScope':'V1全60來源角度比對，10張受影響AI原生重製並逐張核對；其餘來源相同保留。V4原無上櫃保持。'})
