'use strict';
// 4 versions x 12 spaces x 3 authored camera directions, from real source meshes.
...
const titles=['圓弧中島酒吧＋玄關矮櫃','旋轉電視＋小中島','旋轉電視＋大中島','大中島'];
const common={
...
function views(v){
 ...
 return Object.keys(names).flatMap(room=>data[room].map(([p,t],i)=>({id:room+'-'+['A','B','C'][i],room,name:(room==='collection'&&v>=3?'開放收藏收納':names[room])+'・視角 '+['A','B','C'][i],p,t,fov:['closet','bath1','bath2','storage','back'].includes(room)?86:74,direction:Math.atan2(t[1]-p[1],t[0]-p[0])*180/Math.PI})));
}
...
(async()=>{
 ...
 const selected=filter?[Number(filter.split('=')[1])]:[1,2,3,4];
 ...
 for(const v of selected){
  const f=await build(v+1);f.c.HOME_COMFORT.setScene('daily');f.tick(3);
  ...
  for(const q of authored){
   ...
   old.entries.push({...record,key,version:'v'+v,versionTitle:titles[v-1],model:'model/'+file,modelHash:sha(target),angle:q.id.slice(-1),width:1152,height:768,ai:null,status:'source-captured'});
   fs.writeFileSync(jobsFile,JSON.stringify(old,null,2)+'\n');
   console.log(key+' '+record.triangles+' triangles');
  }
 }
 console.log('Captured '+old.entries.length+' / 144 views.');
