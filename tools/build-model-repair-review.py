...
current={e['key']:e['aiKey'] for e in json.loads((A/'album-manifest.json').read_text(encoding='utf8'))['entries']}
aihtml='<section><h2>修正後 AI 概念示意</h2><p>AI 用於材質與光影溝通；配置和尺度請切回模型核對。窗外景色、反射、燈光強弱與五金細節均非現場承諾。</p><div class="pair">'+''.join(f'<figure><img loading="lazy" src="../../成品圖集/20260914暗色現代工業/images/{current[k]}.webp" alt="{k} AI概念示意"><figcaption>{dict((c[0],c[1]) for c in cards)[k]}・AI 示意</figcaption></figure>' for k in ai)+'</div></section>'
