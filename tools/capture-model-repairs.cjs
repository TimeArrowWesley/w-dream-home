...
const out=path.resolve('調整紀錄/20260921四版動線修正'),folder=path.resolve('成品圖集/20260914暗色現代工業'),manifest=JSON.parse(fs.readFileSync(fs.existsSync(path.join(out,'album-before.json'))?path.join(out,'album-before.json'):path.join(folder,'album-manifest.json'),'utf8'));
const sha=p=>crypto.createHash('sha256').update(fs.readFileSync(p)).digest('hex');fs.mkdirSync(path.join(out,'model'),{recursive:true});
(async()=>{const captures=[];for(let v=1;v<=4;v++){
 const f=await build(v+1);f.c.HOME_REALISM.render=()=>{};f.c.HOME_REALISM.invalidate=()=>{};f.c.HOME_COMFORT.setScene('daily');if(v===1){f.c.HOME_INTERACTION.setDoor('study-slide-r05',true);f.tick(80,50);}else f.tick(3);
 for(const e of manifest.entries.filter(e=>e.version==='v'+v)){
  ...
  const view={id:e.key.slice(3)+'-mr01',room:e.room,name:e.name,p:e.p,t:e.t,fov:e.fov,direction:e.direction};
