...
(async()=>{
 ...
 for(const q of before.entries.filter(q=>q.version===v)){
  ...
  const id=q.room+'-'+(q.room==='entry'?'door5':'')+q.angle+'-au01';
  ...
  records.push({...q,...rec,model:'model/'+rec.file,modelHash:h,changed:h!==q.modelHash,previousModelHash:q.modelHash,previousAI:q.aiKey,sourceRevision:'20260923-au01',status:'source-captured'});
