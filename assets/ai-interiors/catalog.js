...
(() => {
  const photo = (room,key,label) => ({id:key,room,label,src:`assets/ai-interiors/${key}-20260910.png`});
  const common = [
  ...
  window.HOME_AI_PHOTOS = Object.fromEntries(['v1','v2','v3','v4'].map(version => {
    const rotating=['v3','v4'].includes(version),low=['v2','v4'].includes(version);
    return [version,[
      photo('entry','entry-'+version,'玄關・'+(low?'90cm 矮櫃':'高矮櫃')),
      photo('living',rotating?'living-rotating':'living-'+version,rotating?'客廳・旋轉電視':'客廳・L 型沙發'),
